#!/usr/bin/env python3
"""Download a Class-C YUV/Y4M source and materialize exactly N YUV420 frames."""
...
def extract_y4m(source_path: Path, output_path: Path, frames: int) -> None:
    with source_path.open("rb") as source, output_path.open("wb") as output:
        ...
        if b"W832" not in header or b"H480" not in header:
...
def extract_raw(source_path: Path, output_path: Path, frames: int) -> None:
    ...
    if source_path.stat().st_size < expected_size:
        raise ValueError(
            f"raw YUV is too small: {source_path.stat().st_size} bytes; "
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        downloaded = Path(temporary_directory) / "class_c_source"
