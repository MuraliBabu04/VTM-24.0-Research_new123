...
RESULT_DIR="${RESULT_DIR:-$ROOT_DIR/results/BitExact_Class_C/$SEQUENCE/QP$QP}"
...
test -f "$ANALYZER" || { echo "Analyzer not found: $ANALYZER" >&2; exit 6; }
...
test -s "$SPARSITY_CSV" || { echo "Sparsity CSV was not produced" >&2; exit 7; }
test -s "$VERIFY_CSV" || { echo "Bit-exact verification CSV was not produced" >&2; exit 8; }
...

{
  ...
  echo
  echo "Anchor bitrate / PSNR:"
  echo "$ANCHOR_METRICS"
  echo
  echo "Proposed bitrate / PSNR:"
  echo "$PROPOSED_METRICS"
  echo
  cat "$VERIFY_SUMMARY"
  echo
  cat "$HASHES"
