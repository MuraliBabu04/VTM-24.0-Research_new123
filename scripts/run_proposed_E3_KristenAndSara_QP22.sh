#!/usr/bin/env bash
# Linux GitHub Actions runner for E3 KristenAndSara, QP22, 64 frames.
set -euo pipefail
...
OUT="${PROPOSED_OUTPUT_DIR:-$ROOT/results/proposed/E3_KristenAndSara/QP22}"
...
"$ENCODER" \
  ...
  --MTS=1 \
  --LFNST=1 \
  -b "$CODEC_OUT/E3_KristenAndSara_QP22.vvc" \
  2>&1 | tee "$CODEC_OUT/E3_KristenAndSara_QP22.log"
...
python3 "$SIMULATOR" \
  ...
  --mts 1 \
  --lfnst 1 \
  2>&1 | tee "$HARDWARE_OUT/E3_KristenAndSara_QP22_power_latency.log"

{
  ...
  grep -E "SUMMARY|Total Frames|^[[:space:]]*[0-9]+[[:space:]]+a[[:space:]]" \
    "$CODEC_OUT/E3_KristenAndSara_QP22.log" || true
  echo
  cat "$HARDWARE_OUT/E3_KristenAndSara_QP22_power_latency.log"
} | tee "$OUT/E3_KristenAndSara_QP22_combined_summary.txt"
